...
function Fact({
  ...
  async function handleVoteInteresting() {
    ...
    if (!error) setFacts((fact) => fact.map((item) => (item.id === id ? updatedFact[0] : item)))
  }
  ...
  return (
    <li className='list__fact-list'>
      <p>
        ...
        {text}
        <a className='list__fact-source' href='https://opensource.fb.com/' target='_blank'>
          (Source)
        </a>
      </p>
      <span
        ...
        style={{
          backgroundColor: categories.find((item) => item.name === category).color,
        }}>
        {category}
      </span>
